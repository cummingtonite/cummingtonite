...
// Copyright (c) 2014 The Cummingtonite developers
...
#if defined(NDEBUG)
# error "Cummingtonite cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1c642eae22bd849b0f8a6cd016270409898e6ef1fa6e48a7dba1118fedb3524b");
uint256 hashGenesisBlockTestNet("0x00000c13e78b90f61b112295b5fb8e9a083830bfc21515ee11dc2e0d35313220");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cummingtonite: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cummingtonite Signed Message:\n";
...
{
    // Cummingtonite: IsDust() detection disabled, allows any valid dust to be relayed.
...
const int64 nTargetSpacing = 60; // Cummingtonite: 1 minute block
static const int64 nTargetTimespan = 60 * 60; // Cummingtonite: Not used
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax)
{
    ...
    {
        // difficulty is reset at the first PoS blocks
        if (fTestNet)
...
void ThreadScriptCheck() {
    RenameThread("cummingtonite-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if (GetHash() == hashGenesisBlock) {
        ...
        pindexGenesisBlock->nMoneySupply = 69 * COIN;
        return true;
    }
...
{
    // Cummingtonite: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The best coin ever made with one hand";
        CTransaction txNew;
        txNew.nVersion = 1;
        txNew.nTime = 1411377980;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1411377980;
        block.nBits = 0x1e0ffff0;
        block.nNonce = 3060420;

        if (fTestNet)
        {
            txNew.nTime = block.nTime = 1411377980;
            block.nNonce = 1107143;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x6007ecc470782b15f941a0391dd50427d1ab646c009f6442da52c58fd261578e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cummingtonite: increase each by adding 2 to bitcoin's value.
